...
public class InputsHandler {
    ...
    public static Inputs checkAndExtractRefInputs(XMCDA xmcda, ProgramExecutionResult xmcdaExecResults, InputsHandler.Inputs inputsPrev) {
        Inputs inputsDict = checkRefInputs(xmcda, xmcdaExecResults, inputsPrev);
        if (xmcdaExecResults.isError())
            return null;
        return inputsDict;
    }
    ...
    protected static Inputs checkInputs(XMCDA xmcda, ProgramExecutionResult errors) {
        ...
        try {
            ...
            checkAndExtractCategoriesAssignments(inputs, xmcda, errors);
    ...
    protected static Inputs checkRefInputs(XMCDA xmcda, ProgramExecutionResult errors, InputsHandler.Inputs inputsPrev) {
        Inputs inputs = new Inputs();

        try {
            checkAndExtractAlternativesRef(inputs, xmcda, errors, inputsPrev);
            checkAndExtractTableRef(inputs, xmcda, errors, inputsPrev);
            computeCrriteriaNumericValues(inputsPrev);
            addCriteriaToVariantsRef(inputs, inputsPrev);
            createMatrixRef(inputs, inputsPrev);
    ...
    protected static void checkAndExtractAlternatives(Inputs inputs, XMCDA xmcda, ProgramExecutionResult errors) throws InputDataException {
        ...
        for(String alternativeId: alternativesIds) {
            if(inputs.getVariantsNameArray().contains(alternativeId)) {
                String errorMessage = "There is already alternative with the given id";
                errors.addError(errorMessage);
                throw new InputDataException(errorMessage);
            }
    ...
    protected static void checkAndExtractAlternativesRef(Inputs inputs, XMCDA xmcda, ProgramExecutionResult errors, InputsHandler.Inputs inputsPrev) throws InputDataException {
        if (xmcda.alternatives.isEmpty()) {
            String errorMessage = "No alternatives list has been supplied";
            errors.addError(errorMessage);
            throw new InputDataException(errorMessage);
        ...
        ArrayList<String> alternativesNameArray = new ArrayList<String>();
        for(String alternativeId: alternativesIds) {
            if(inputsPrev.getVariantsNameArray().contains(alternativeId)) {
                String errorMessage = "There is already alternative in reference set with the given id";
                errors.addError(errorMessage);
                throw new InputDataException(errorMessage);
            }
            Variant newVariant = new Variant(alternativeId);
    ...
    protected static void checkAndExtractCriteria(Inputs inputs, XMCDA xmcda, ProgramExecutionResult errors) throws InputDataException {
        ...
        for(String criteriaId: criteriaIds) {
            if(inputs.getCriteriaNameArray().contains(criteriaId)) {
                String errorMessage = "There is already criteria with the given id";
                errors.addError(errorMessage);
                throw new InputDataException(errorMessage);
            }
    ...
    protected static void checkAndExtractTableRef(Inputs inputs, XMCDA xmcda, ProgramExecutionResult errors, InputsHandler.Inputs inputsPrev) throws InputDataException {
        if (xmcda.performanceTablesList.size() != 1) {
            String errorMessage = "There should be only one performance table";
            errors.addError(errorMessage);
            throw new InputDataException(errorMessage);
        ...
        for (Alternative alternative : alternativesArray) {
            ...
            for (Criterion criterion : criteriaArray) {
                if(!inputsPrev.getCriteriaNameArray().contains(criterion.id())) {
                    String errorMessage = "Criterion in performance table doesn't exists";
                    errors.addError(errorMessage);
                    throw new InputDataException(errorMessage);
                }
                if(inputsPrev.getCriteriaMap().get(criterion.id()).getType().equals("numerical")) {
                    if(!(profilesPerformance.getValue(alternative, criterion) instanceof Integer || profilesPerformance.getValue(alternative, criterion) instanceof Double)) {
                        String errorMessage = "Criterion type in performance table isn't compatible with scale";
                        errors.addError(errorMessage);
                        throw new InputDataException(errorMessage);
                ...
                if(inputsPrev.getCriteriaMap().get(criterion.id()).getType().equals("ordinal")) {
                    if(!(profilesPerformance.getValue(alternative, criterion) instanceof String)) {
                        String errorMessage = "Criterion type in performance table isn't compatible with scale";
                        errors.addError(errorMessage);
                        throw new InputDataException(errorMessage);
                ...
                if(inputsPrev.getCriteriaMap().get(criterion.id()).getType().equals("numerical")) {
                    if(!(inputsPrev.getCriteriaMap().get(criterion.id()).getOriginalValues().contains(String.valueOf(value)))) {
                        inputsPrev.getCriteriaMap().get(criterion.id()).getOriginalValues().add(String.valueOf(value));
                    }
                }
            }
        }
        sortNumericalCriteria(inputsPrev);
    }
    ...
    protected static void checkAndExtractCategoriesAssignments(Inputs inputs, XMCDA xmcda, ProgramExecutionResult errors) throws InputDataException {
        ...
        if (xmcda.alternativesAssignmentsList.size() > 1) {
            String errorMessage = "More than one alternatives assignments list has been supplied";
            errors.addError(errorMessage);
            throw new InputDataException(errorMessage);
        }

        AlternativesAssignments alternativeAssignments = xmcda.alternativesAssignmentsList.get(0);
        if(alternativeAssignments.size() < inputs.getVariantsNameArray().size()) {
            String errorMessage = "You need to provide assignments to all alternatives";
            errors.addError(errorMessage);
            throw new InputDataException(errorMessage);
        }
        ArrayList<String> readAlternativesNames = new ArrayList<>();
        for(Object alternativeAssignment: alternativeAssignments) {
            AlternativeAssignment assignment = (AlternativeAssignment)alternativeAssignment;

            if(!inputs.getVariantsNameArray().contains(assignment.getAlternative().id())) {
                String errorMessage = "Alternative in assignments doesn't exists";
                errors.addError(errorMessage);
                throw new InputDataException(errorMessage);
            }
            if(readAlternativesNames.contains(assignment.getAlternative().id())) {
                String errorMessage = "There are two assignments to the same alternative";
                errors.addError(errorMessage);
                throw new InputDataException(errorMessage);
            }
            readAlternativesNames.add(assignment.getAlternative().id());

            if(!inputs.getCriteriaMap().get("decision").getOriginalValues().contains(assignment.getCategory().id())) {
                String errorMessage = "Category in assignments doesn't exists";
                errors.addError(errorMessage);
                throw new InputDataException(errorMessage);
            }
            inputs.getVariantsMap().get(assignment.getAlternative().id()).getValuesMap().put("decision",assignment.getCategory().id());
        }
    }
    ...
    protected static void addCriteriaToVariantsRef(Inputs inputs, Inputs inputsPrev) {
        for(Map.Entry<String,Variant> variant: inputs.variantsMap.entrySet()) {
            variant.getValue().setAttributesMap(inputsPrev.getCriteriaMap());
        }
    }
    ...
    protected static void createMatrixRef(Inputs inputs, Inputs inputsPrev) {
        inputs.setMatrix(new Matrix(inputsPrev.getCriteriaMap(), inputs.getVariantsMap()));
    }
