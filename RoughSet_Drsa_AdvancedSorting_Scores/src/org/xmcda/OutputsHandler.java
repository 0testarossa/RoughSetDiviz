...
public class OutputsHandler {
    ...
    public static Map<String, XMCDA> convertSortInputExamples(Map<Pair, Double> alternativesSupport)
    {
        final HashMap<String, XMCDA> xResults = new HashMap<>();
        ...
        for (Map.Entry<Pair, Double> alternativeEntry : alternativesSupport.entrySet()) {
            Category category = new Category(alternativeEntry.getKey().getElementRight());
            AlternativeAssignment<Double> tmpAssignment = new AlternativeAssignment<>();

            QualifiedValue<Double> value = new QualifiedValue<>();
            QualifiedValues<Double> valuesSet = new QualifiedValues<>();
